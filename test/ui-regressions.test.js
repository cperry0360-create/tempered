...
test('REGRESSION: Health import has a manual paste path when iOS denies clipboard reads', () => {
  const health = read('src/ui/health-shortcut-runtime.js')
  const nutrition = read('src/ui/calorie-ai-runtime.js')
  const css = read('src/pivot.css')
  ...
  assert.match(health, /touch and hold in the box and tap Paste/i)
  assert.match(health, /const sheet = openImportSheet\(button\)/)
  assert.match(health, /if \(!sheet\.overlay\.isConnected\) return/)
  assert.match(health, /addEventListener\('tempered:screen-shown'/)
  assert.match(health, /addEventListener\('tempered:today-rendered'/)
  assert.match(health, /addEventListener\('tempered:lifestyle-ready'/)
  assert.doesNotMatch(health, /new MutationObserver/)
  assert.match(nutrition, /addEventListener\('tempered:screen-shown'/)
  assert.match(nutrition, /addEventListener\('tempered:today-rendered'/)
  assert.match(nutrition, /tempered:lifestyle-ready/)
  assert.doesNotMatch(nutrition, /new MutationObserver/)
  assert.match(css, /\.health-import-overlay/)
  assert.match(css, /\.health-import-sheet__input/)
})
